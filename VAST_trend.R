...
# 0. Prepare the data -------------------------------------------
dirname = "/Users/Yuki/Dropbox/Network2020"
setwd(dir = dirname)
...
# read the data
df = read.csv("VASTdata.csv", fileEncoding = "CP932")
summary(df)
df = df %>% 
  filter(Trend == "inc", Y != 1987, Y != 2019) %>% 
  select(Y, Lon, Lat, CPUE, FISH2) %>% 
  rename(lat = Lat, lon = Lon, year = Y, spp = FISH2, cpue = CPUE)
summary(df)

# check = group_by(year, spp) %>% summarize(count = n())
...
# 1.6 Save settings
DateFile = paste0(dirname,'/test_inc/')
dir.create(DateFile)
...
n_spp = data.frame(spp = unique(Data_Geostat$spp), spp2 = rep(1:length(unique(Data_Geostat$spp))))
Data_Geostat = merge(Data_Geostat, n_spp, by = "spp")
TmbData = make_data(
  Version = Version,
...
FishStatsUtils::plot_factors(Report = Report,
                             ...
                             Year_Set = Year_Set,
                             category_names = levels(Data_Geostat[, "spp2"]),
                             plotdir = DateFile)
